...
for file in all_directory:
    if osp.isfile(file) and file.startswith('equil') and file.endswith(file_extension):# and file.startswith('prod'):
...
for count,file in enumerate(file_list,0):
    ...
    axs[0,0].set_title('Si-Si')
    axs[0,0].set_ylabel('g(r)')
    axs[0,1].plot(total_rdf[:,0],total_rdf[:,2])
    axs[0,1].set_title('Si-O')
    axs[0,1].set_ylabel('g(r)')
    axs[0,2].plot(total_rdf[:,0],total_rdf[:,3])
    axs[0,2].set_title('O-O')
